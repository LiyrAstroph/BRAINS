...
void line_gaussian_smooth_2D_FFT(const double *transv, double *fl2d, int nl, int nv, const void *pm)
{
  ...
  {
    sigV_instrinsic = parset.width_narrowline + pmodel[num_params_blr-num_params_res-num_params_linecenter-3-2] * parset.width_narrowline_err;
  }
  
  if(parset.InstRes >= 0.0)
  {
    sigV = parset.InstRes + pmodel[num_params_blr-num_params_res-num_params_linecenter-3]*parset.InstRes_err;
  ...
  else
  {
    for(j=0; j<nl; j++)
    {
      sigV = instres_epoch[j] + pmodel[num_params_blr-num_params_res-num_params_linecenter-3 + j]*instres_err_epoch[j];
      sigV = fmax(0.0, sigV*sigV - sigV_instrinsic*sigV_instrinsic);
      sigV = sqrt(sigV);
